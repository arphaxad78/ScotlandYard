function GameState() {
  ...
  //var test = getStartingPositions
  var start_positions = GAME_LOGIC.getStartingPositions();
  // the first number is Mr.X and the last 4 are the players

  this.mrx_location = start_positions[0];
  ...
  this.red_player_location = start_positions[1];
  this.yellow_player_location = start_positions[2];
  this.green_player_location = start_positions[3];
  this.blue_player_location = start_positions[4];
...
function DoesMrXShowThisTurn(turn) {
  // this is needed for a call in the graphics.js file
...
function PlayNextTurn(thisGameState, thisTurn) {
  ...
  if (thisGameState.turn > 0) {
    ...
    if (IsThisAValidMove2(thisGameState.red_player_location, thisTurn.red_player_location) != true) {
      thisGameState.message = "The red player's move is invalid.";
      return 2;
    }
    if (IsThisAValidMove2(thisGameState.yellow_player_location, thisTurn.yellow_player_location) != true) {
      thisGameState.message = "The yellow player's move is invalid.";
      return 2;
    }
    if (IsThisAValidMove2(thisGameState.green_player_location, thisTurn.green_player_location) != true) {
      thisGameState.message = "The green player's move is invalid.";
      return 2;
    }
    if (IsThisAValidMove2(thisGameState.blue_player_location, thisTurn.blue_player_location) != true) {
    ...
    var these_positions = [];
    these_positions.push(thisGameState.mrx_location);
    these_positions.push(thisGameState.red_player_location);
    these_positions.push(thisGameState.yellow_player_location);
    these_positions.push(thisGameState.green_player_location);
    these_positions.push(thisGameState.blue_player_location);

    // now let's see if they moved on Mr. X
    if (GAME_LOGIC.isMrXCaught(these_positions)) {
      thisGameState.message = "A player discovered Mr. X at location " + thisGameState.mrx_location + "!";
      return 1;
    }
  ...
  var positions = [];
  positions.push(thisGameState.mrx_location);
  positions.push(thisGameState.red_player_location);
  positions.push(thisGameState.yellow_player_location);
  positions.push(thisGameState.green_player_location);
  positions.push(thisGameState.blue_player_location);

  var mrx_move = GAME_LOGIC.moveMrX(positions, thisTurn);
  ...
  thisGameState.mrx_location = mrx_move.position;
...
function IsThisAValidMove2(source, destination) {
  // I don't care what transportation is used, but Dan's call does...
  // ...so try 0, 1, and 2 (the non-boat routes) and if any of them work, then return true
  var return_bool = false;
  for (i = 0; i <= 2; i++) {
    if (GAME_LOGIC.isMoveValid(source, destination, i)) {
      return_bool = true;
    }
  }
  return return_bool;
...
// things I need for game logic are above this line
// things below I need for graphics or other underlying stuff
...
// *****these ones below have been officially deprecated*****
